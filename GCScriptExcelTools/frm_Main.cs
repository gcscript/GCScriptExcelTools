...
public partial class frm_Main : Form
{
    ...
    private void btn_Start_Click(object sender, EventArgs e)
    {
        ...
        foreach (var item in lst_FilesPath.Items)
        {
            ...
            string firstCaracter = "_";
            string finalPath = string.Empty;

            while (true)
            {
                finalPath = Path.Combine(filePath!, $"{firstCaracter}{fileName}.xlsx");
                if (finalPath.Length > 255)
                {
                    MessageBox.Show($"O arquivo [{fileName}] excedeu o limite de caracteres!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    return;
                }

                if (File.Exists(finalPath))
                {
                    firstCaracter = firstCaracter + "_";
                    continue;
                }
                else
                {
